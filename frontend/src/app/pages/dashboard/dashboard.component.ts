...
export class DashboardComponent implements OnInit, OnDestroy {
    ...
    try {
      ...
      timerStates.forEach(timerState => {
        ...
        if (task) {
          ...
          if (timerState.isCountdown && elapsedMinutes >= timerState.timeEstimate) {
  ...
  private loadAnalytics() {
    ...
    const analyticsSub = this.http.get<Analytics>('http://localhost:8000/analytics', {
  ...
  private convertBackendTasksToFrontend(backendTasks: Task[]): FrontendTask[] {
    return backendTasks.map(task => ({
      ...
      dueDate: task.due_date ? new Date(task.due_date).toISOString().split('T')[0] : this.formatDate(new Date()),
  ...
  private convertFrontendTaskToBackend(frontendTask: FrontendTask): any {
    return {
      ...
      due_date: frontendTask.dueDate ? new Date(frontendTask.dueDate).toISOString() : null,
  ...
  applyFilters() {
    ...
    filterTasksByStatus() {
    this.notStartedTasks = this.filteredTasks.filter(task => task.status === 'Not Started');
    this.inProgressTasks = this.filteredTasks.filter(task => task.status === 'In Progress');
    this.pendingTasks = this.filteredTasks.filter(task => task.status === 'Pending');
    ...
    const config: ChartConfiguration = {
      ...
      data: {
        ...
        datasets: [{
          ...
          data: this.analytics.productivity_trend.map(item => item.completed_tasks),
          borderColor: '#3498db',
  ...
  updateTask() {
    ...
    const updateSub = this.taskService.updateTask(this.selectedTask.id!, backendTask).subscribe({
      next: (updatedTask) => {
        ...
        if (index !== -1) {
          ...
          this.tasks[index] = this.convertBackendTasksToFrontend([updatedTask])[0];
  ...
  private updateTaskInBackend(task: FrontendTask) {
    ...
    const updateSub = this.taskService.updateTask(task.id!, backendTask).subscribe({
      next: () => {
        this.loadAnalytics();
      },
      error: (error) => {
  ...
  startTimer(task: FrontendTask, isCountdown: boolean = false) {
    ...
    if (isCountdown && (!task.timeEstimate || task.timeEstimate <= 0)) {
      const estimateMinutes = prompt('Enter time estimate in minutes for countdown timer:', '25');
  ...
  getRealTimeCountdown(task: FrontendTask): string {
    ...
    const remainingMinutes = Math.max(0, (task.timeEstimate || 0) - totalElapsedMinutes);
    const remainingSeconds = remainingMinutes > 0 ? 60 - elapsedSeconds : 0;

    const actualRemainingMinutes = remainingSeconds === 60 ? remainingMinutes : remainingMinutes;
    const actualRemainingSeconds = remainingSeconds === 60 ? 0 : remainingSeconds;
  ...
  estimateTimeFromDescription(description: string): number {
    ...
    const complexKeywords = ['research', 'analyze', 'develop', 'create', 'design', 'implement'];
    const simpleKeywords = ['update', 'fix', 'review', 'call', 'email', 'check'];
    
    const hasComplexKeywords = complexKeywords.some(keyword => description.toLowerCase().includes(keyword));
    const hasSimpleKeywords = simpleKeywords.some(keyword => description.toLowerCase().includes(keyword));
  ...
  startTimerForMultipleTasks(tasks: FrontendTask[], isCountdown: boolean = false) {
    tasks.forEach(task => {
  ...
  getTimeTrackingSummary() {
    const totalEstimated = this.tasks.reduce((sum, task) => sum + (task.timeEstimate || 0), 0);
    const totalSpent = this.tasks.reduce((sum, task) => sum + (task.timeSpent || 0), 0);
