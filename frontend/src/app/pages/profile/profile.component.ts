...
export class ProfileComponent implements OnInit {
  ...
  countries = [
    'South Africa', 'Canada', 'United Kingdom', 'Australia', 
    'Germany', 'France', 'Japan', 'China', 'India', 'Brazil',
    'United States', 'Nigeria', 'Kenya', 'Egypt', 'Morocco',
    'Mexico', 'Spain', 'Italy', 'Russia', 'South Korea'
  ];
  ...
  loadProfile(): void {
    ...
    const savedProfile = localStorage.getItem(this.getUserStorageKey('userProfile'));
    if (savedProfile) {
  ...
  onSubmit(): void {
    ...
    setTimeout(() => {
      if (this.currentUserId) {
        localStorage.setItem(this.getUserStorageKey('userProfile'), JSON.stringify(this.profileForm.value));
        localStorage.setItem(this.getUserStorageKey('userEmail'), this.profileForm.value.email);
      }
    ...
    fileInput.onchange = (e) => {
      ...
      if (target.files && target.files.length && this.currentUserId) {
        ...
        reader.onload = (event) => {
          ...
          localStorage.setItem(this.getUserStorageKey('profilePicture'), this.profilePicture);
        };
  ...
  hasError(field: string, errorType: string): boolean {
    return this.profileForm.get(field)?.hasError(errorType) && 
           (this.profileForm.get(field)?.touched || this.submitted) || false;
  }
  ...
  initSecurityForms(): void {
    ...
    let currentEmail = '';
    if (this.currentUserId) {
      currentEmail = localStorage.getItem(this.getUserStorageKey('userEmail')) || '';
    }
  ...
  hasSecurityError(form: FormGroup, field: string, errorType: string): boolean {
    ...
    const isSubmitted = field === 'newEmail' ? this.emailSubmitted : 
                        field === 'newPassword' || field === 'confirmPassword' ? this.passwordSubmitted : 
                        this.deleteSubmitted;
    
    return (control?.hasError(errorType) && (control?.touched || isSubmitted)) || false;
  }
