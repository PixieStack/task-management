...
export class AuthService {
  ...
  isLoggedIn(): boolean { const token = this.getToken(); const expiresAt = sessionStorage.getItem('expires_at'); if (!token || !expiresAt) return false; const valid = Date.now() < new Date(expiresAt).getTime(); if (!valid) this.clearSession(); return valid; }
  ...
  updateSessionUser(user: User): void { sessionStorage.setItem('username', user.username); sessionStorage.setItem('userEmail', user.email); sessionStorage.setItem('userId', String(user.id)); this.userSubject.next(user); }
  invalidateSession(): void { this.clearSession(); }
  private setSession(result: AuthResponse): void { sessionStorage.setItem('token', result.access_token); sessionStorage.setItem('expires_at', new Date(Date.now() + result.expires_in * 1000).toISOString()); this.updateSessionUser(result.user); }
  private checkAuthStatus(): void { if (!this.isLoggedIn()) { this.clearSession(); return; } const id = Number(sessionStorage.getItem('userId')); const username = sessionStorage.getItem('username'); const email = sessionStorage.getItem('userEmail'); if (id && username && email) this.userSubject.next({ id, username, email }); }
  private clearSession(): void {
    const keys = ['token','username','userEmail','expires_at','userId'];
    keys.forEach((key) => {
